...
const NewsApperanceTable = ({ data, refetchData }) => {
  const apiUrl = `${import.meta.env.VITE_APP_API_URL}/news-apperances`;
      ...
      <table className='min-w-full divide-y divide-gray-200'>
        <thead className='bg-gray-50'>
          <tr>
            ...
            <th
              scope='col'
              className='md:w-[15rem] capitalize px-4 py-3.5 text-left text-sm font-bold text-gray-700'
            >
              <span>Title</span>
            </th>
            <th
            ...
            <tr key={index}>
              ...
              <td className='whitespace-wrap px-5 py-4'>
                <div className='capitalize font-semibold line-clamp-1 md:line-clamp-3 text-sm w-[100%] text-gray-900 browser-css'>
                  <Link to={`/news-apperances/detail/${item?.id}`}>
                    {item?.title}
                  </Link>
                </div>
              </td>
              ...
              <td className='whitespace-wrap px-5 py-4'>
                <div className='text-sm w-[100%] text-gray-900 line-clamp-4 browser-css'>
                  {formateDate(item?.publishedDate)}
                </div>
              </td>
              <td className='whitespace-wrap px-5 py-4'>
                <div className='text-sm w-[100%] text-gray-900 line-clamp-4 browser-css'>
              ...
              <td className='whitespace-wrap px-12 py-5 flex items-center justify-center gap-3'>
                <div className='text-sm text-gray-900 '>
                  <ToolTip text='edit'>
                    <Link to={`/news-apperances/edit/${item.id}`}>
...
export default NewsApperanceTable;
