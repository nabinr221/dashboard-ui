...
const ProjectMemberTable = ({ data, refetchData }) => {
  const apiUrl = `${import.meta.env.VITE_APP_API_URL}/project-member`;
          ...
          {data?.map((item, index) => (
            <tr key={index}>
              ...
              <td className='whitespace-wrap px-4 py-4'>
                <div className='text-sm font-semibold  capitalize line-clamp-1 sm:line-clamp-3 overflow-hidden '>
                  {item?.project?.title}
                </div>
              </td>
              ...
              <td className='w-[9rem] px-4 py-4'>
                <div className='text-sm font-medium text-black capitalize line-clamp-3 overflow-hidden  '>
                  {item?.category === "teamLead" && "Team lead"}
                  {item?.category === "member" && "member"}
                </div>
              </td>
              ...
              <td className='whitespace-wrap px-12 w-full py-6 flex items-center justify-center gap-3'>
                <div className='text-sm text-gray-900 '>
                  <ToolTip text='edit'>
                    <Link to={`/project-members/edit/${item.id}`}>
