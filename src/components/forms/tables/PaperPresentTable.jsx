...
const PaperPresentTable = ({ data, refetchData }) => {
  const apiUrl = `${import.meta.env.VITE_APP_API_URL}/paper/presents`;
    ...
    <TableContainer>
      <table className='min-w-full divide-y divide-gray-200'>
        ...
        <tbody className='divide-y divide-gray-200 bg-white'>
          {newData?.map((paperPresent, index) => (
            <tr key={index}>
              ...
              <td className='whitespace-wrap px-4 py-4'>
                <div className='text-sm font-medium text-black capitalize'>
                  {paperPresent.organization}
                </div>
              </td>
              <td className='whitespace-wrap px-4 py-4'>
                <div className='text-sm font-medium text-black capitalize'>
                  {paperPresent.subject}
                </div>
              </td>
              <td className='whitespace-wrap px-4 py-4 text-sm text-gray-700'>
                <div className='text-sm text-gray-700 line-clamp-3'>
                  {paperPresent.description}
                </div>
              </td>

              <td className='whitespace-wrap px-4 py-4 text-sm text-gray-700'>
                <div className='text-sm text-gray-700'>{paperPresent.date}</div>
              </td>

              <td className='whitespace-wrap px-12 py-4 flex items-center justify-center gap-3'>
                <div className='text-sm text-gray-900 '>
                  <ToolTip text='edit'>
                    <Link to={`/paper-present/edit/${paperPresent._id}`}>
                ...
                <div className='text-sm text-gray-700'>
                  <ToolTip text='delete'>
                    <span
                      onClick={() => openModal(paperPresent._id)}
...
export default PaperPresentTable;
